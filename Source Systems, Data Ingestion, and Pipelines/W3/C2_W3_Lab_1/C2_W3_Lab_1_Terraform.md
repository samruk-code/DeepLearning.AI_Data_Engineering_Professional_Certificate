...
by clicking the copy button beside it. Replace the placeholder with the Account ID that you copied. You can also find it in the **CloudFormation** **Outputs** tab.
...
- In the bastion host `outputs.tf` file
  ...
  - Bastion host public DNS: also refer to the `aws_instance` resource you
    created in [`ec2.tf`](terraform/modules/bastion_host/ec2.tf). You can look for the name of
  ...
  - RDS host (or address): refer to the `aws_db_instance` resource you created
    in `rds.tf`.  You can look for the name of the host/address attribute
    [here](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/resources/db_instance#attribute-reference).
  - RDS port: same as previous comment.
  - Database Username: same as previous comment.
