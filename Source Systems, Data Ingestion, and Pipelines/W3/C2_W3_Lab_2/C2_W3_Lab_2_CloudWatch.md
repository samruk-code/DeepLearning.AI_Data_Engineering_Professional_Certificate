...
`Ctrl+C` or typing `quit` and pressing Enter). You can now close this tab.
...
1 to 100). You can use hotkeys `Ctrl+/` or `Cmd+/`. Alternatively, if this doesn't work you can click on the beginning of the first line, then `Shift+Alt+Click` at the beginning of the last line you want to comment. Then hit `delete` twice, this should remove the first two characters (`# `) from all the selected lines. Save changes to the file.

In this resource, there is a key named `dashboard_body` (line 13); 
...
*Note*: if you closed the terminal inside VSCode and need to open a new one, remember to run the following commands before attempting to plan and apply: 
...
Uncomment the resources associated with the **Alerts** (lines 102 to 215). Save
changes. 
...
(lines 217 to 258) and complete the code associated with the resource `RdCpuUsage`: 
