...
[`GXValidateDataFrameOperator`](https://great-expectations.github.io/airflow-provider-great-expectations/latest/getting-started/#data-frame-operator).
...
2. Complete the definition of the `GXValidateDataFrameOperator` in the section 
associated with the `EXERCISE 1`. You will need to configure the expectation for `passenger_count`, which should be between 1 and 6, as well as the `context_type` and `result_format` arguments.
...
deployment of a Machine Learning model, configured a `GXValidateDataFrameOperator` 
